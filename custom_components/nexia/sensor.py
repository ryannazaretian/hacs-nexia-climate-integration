...
from . import (ATTR_MODEL, ATTR_FIRMWARE, ATTR_THERMOSTAT_NAME,
               ATTR_THERMOSTAT_ID, ATTR_ZONE_ID,
               ATTRIBUTION,
...
class NexiaSensor(Entity):
    ...
    def _update(self):
        if self._device.last_update is None or datetime.datetime.now() - \
                self._device.last_update > self._scan_interval:
            self._device.update()
...
class NexiaZoneSensor(NexiaSensor):
    ...
    def __init__(self, device, scan_interval, thermostat_id, zone, sensor_call,
                 sensor_name,
                 sensor_class, sensor_unit,
                 modifier=None):
        super().__init__(device, scan_interval, thermostat_id, sensor_call,
                         sensor_name,
                         sensor_class, sensor_unit,
                         modifier)
        self._zone = zone
